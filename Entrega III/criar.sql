...
CREATE TABLE FuncionarioTerra (
    ...
    CONSTRAINT fk_idFuncionario FOREIGN KEY (idFuncionario) REFERENCES Funcionario (Identificacao) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE FuncionarioAr (
    ...
    idFuncionario     INTEGER REFERENCES Funcionario (Identificacao),
    PRIMARY KEY (
        idFuncionarioAr,
        idFuncionario
    ),
...
CREATE TABLE Voo (
    ...
    Designacao         STRING,
    DuracaoDeVoo       INTEGER,
    Combustivel        INTEGER CHECK (Combustivel > 0),
    TaxaAeroportuaria INTEGER,
...
CREATE TABLE PassagemAerea (
    NBilheteEletronico INTEGER PRIMARY KEY,
    Tarifa             INTEGER,
    idCliente          INTEGER REFERENCES Cliente (Identificacao),
    idAcompanhante     INTEGER REFERENCES Acompanhante (Identificacao),

    CONSTRAINT fk_idCliente FOREIGN KEY (idCliente) REFERENCES Cliente (Identificacao) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_idAcompanhante FOREIGN KEY (idAcompanhante) REFERENCES Acompanhante (Identificacao) ON DELETE CASCADE ON UPDATE CASCADE
);

DROP TABLE  IF EXISTS DataDeVoo;
CREATE TABLE DataDeVoo (
    idPassagemAerea INTEGER REFERENCES PassagemAerea (NBilheteEletronico),
    idVoo           INTEGER REFERENCES Voo (idVoo),
    ...
    CONSTRAINT fk_idPassagemAerea FOREIGN KEY (idPassagemAerea) REFERENCES PassagemAerea (NBilheteEletronico) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_idVoo FOREIGN KEY (idVoo) REFERENCES Voo (idVoo) ON DELETE CASCADE ON UPDATE CASCADE
);
